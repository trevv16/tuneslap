            ...
            >
              {item.mediaType === 'image' ? (
                ...
                <div className={classNames(
                  item.id === selectedItem?.id ? '' : 'group-hover:opacity-75',
                  'pointer-events-none aspect-10/7 bg-gray-200 flex items-center justify-center w-full',
                )}>
                  <div className="text-gray-500 text-2xl">🎵</div>
                </div>
              )}
            </div>
            <p className="pointer-events-none mt-2 block truncate text-sm font-medium text-base text-left">
              {item.fileName}
            </p>
            <p className="pointer-events-none block text-sm font-medium text-neutral-300 text-left">
